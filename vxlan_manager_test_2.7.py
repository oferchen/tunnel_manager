...
class TestVXLANManager(unittest.TestCase):
    ...
    def test_create_vxlan_interface(self, mock_call, mock_popen):
        ...
        mock_call.assert_called_with(['ip', 'link', 'add', 'vxlan' + str(vni), 'type', 'vxlan',
                                     'id', str(vni),
                                     'local', src_host,
                                     'remote', dst_host,
                                     'dev', 'eth0',
                                     'dstport', str(src_port)])
        mock_call.assert_called_with(['ip', 'link', 'set', 'vxlan' + str(vni), 'up'])
        mock_call.assert_called_with(['brctl', 'addif', bridge_name, 'vxlan' + str(vni)])
